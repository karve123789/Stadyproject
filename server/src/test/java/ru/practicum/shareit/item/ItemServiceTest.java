...
public class ItemServiceTest {
    ...
    void setup() {
        ...
        item1 = new Item(1, "Balalaika", "Brand new balalaika", true, user1, request, new BigDecimal("100.00"));
        item2 = new Item(2, "Matryoshka", "A set of 5 dolls", true, user1, null, new BigDecimal("50.00")); // Важно указать владельца и цену
    ...
    void add_shouldReturnItemDto_whenWithoutRequestId() {
        ItemDtoWithRequestId itemDto = new ItemDtoWithRequestId(itemMapper.toItemDto(item1), null); // Создаем ItemDto на основе Item

        Integer userId = user1.getId();

        Mockito.when(userRepository.existsById(userId)).thenReturn(true);
        Mockito.when(userRepository.findById(userId)).thenReturn(Optional.of(user1));
        Mockito.when(itemRepository.save(any(Item.class))).thenReturn(item1); // Указываем тип аргумента
        ...
        assertAll(
                ...
                () -> assertEquals(item1.getPrice(), savedItemDto.getPrice()), // Проверяем цену
                () -> assertNull(savedItemDto.getRequestId())
        );

        verify(itemRepository, times(1)).save(any(Item.class)); // Используем times(1) вместо atMostOnce()
    }


    @Test
    void add_shouldReturnItemDto_whenWithRequestId() {
        ItemDtoWithRequestId itemDtoToSave = new ItemDtoWithRequestId(itemMapper.toItemDto(item1), request.getId());// Создаем ItemDto на основе Item
        Integer userId = user1.getId();

        Mockito.when(requestRepository.existsById(itemDtoToSave.getRequestId())).thenReturn(true);
        Mockito.when(requestRepository.findById(itemDtoToSave.getRequestId())).thenReturn(Optional.of(request));
        Mockito.when(userRepository.existsById(userId)).thenReturn(true);
        Mockito.when(userRepository.findById(userId)).thenReturn(Optional.of(user1));
    ...
    void add_throwsObjectNotFoundException_whenRequestNotFound() {
        ...
        Mockito.when(requestRepository.existsById(itemDtoToSave.getRequestId())).thenReturn(false);
    ...
    void update_shouldReturnItemDtoWithNewNameNewDescriptionAndNewEmail() {
        ...
        Mockito.when(itemRepository.existsById(itemIdToUpdate)).thenReturn(true);
        Mockito.when(itemRepository.findById(itemIdToUpdate)).thenReturn(Optional.of(item1));
        Mockito.when(itemRepository.save(any(Item.class))).thenReturn(item2); // Возвращаем item2 после "обновления"
        ...
        assertAll(
                ...
                () -> assertEquals(item2.getPrice(), savedItemDto.getPrice()) // Проверяем цену
        );
    ...
    void update_throwsObjectNotFoundException_whenItemNotFound() {
        ...
        Mockito.when(itemRepository.existsById(itemIdToUpdate)).thenReturn(false);
    ...
    void get_shouldReturnItemDtoExtended_whenRequestedByBooker() {
        ...
        Mockito.when(itemRepository.existsById(itemIdToGet)).thenReturn(true);
        Mockito.when(itemRepository.findById(itemIdToGet)).thenReturn(Optional.of(item1));
        Mockito.when(commentRepository.findByItemId(itemIdToGet)).thenReturn(List.of());
        ...
        assertAll(
                ...
                () -> assertEquals(item1.getPrice(), savedItemDtoExtended.getPrice()), // Проверяем цену
        ...
        verify(itemRepository, times(1)).findById(itemIdToGet);
    }
    ...
    void get_shouldReturnItemDtoExtendedWithLastAndNextBooking() {
        ...
        Mockito.when(bookingRepository.findByItemIdAndStartIsBeforeAndStatusNot(any(), any(LocalDateTime.class), eq(BookingStatus.REJECTED), any())).thenReturn(List.of(lastBooking));
        Mockito.when(bookingRepository.findByItemIdAndStartIsAfterAndStatusNot(any(), any(LocalDateTime.class), eq(BookingStatus.REJECTED), any())).thenReturn(List.of(nextBooking));
        ...
        assertAll(
                ...
                () -> assertEquals(item1.getPrice(), itemDtoSaved.getPrice()), // Проверяем цену
    ...
    void get_throwsObjectNotFoundException_whenItemNotFound() {
        ...
        Mockito.when(itemRepository.existsById(itemIdToGet)).thenReturn(false);
    ...
    void getAll() {
        ...
        Mockito.when(itemRepository.findByOwnerIdOrderByIdAsc(userId)).thenReturn(List.of(item1));
        Mockito.when(bookingRepository.findByItemIdAndStartIsBeforeAndStatusNot(any(), any(LocalDateTime.class), eq(BookingStatus.REJECTED), any())).thenReturn(List.of(lastBooking));
        Mockito.when(bookingRepository.findByItemIdAndStartIsAfterAndStatusNot(any(), any(LocalDateTime.class), eq(BookingStatus.REJECTED), any())).thenReturn(List.of(nextBooking));
        ...
        assertAll(
                ...
                () -> assertEquals(item1.getPrice(), allUserItems.get(0).getPrice()), // Проверяем цену
                () -> assertNotNull(allUserItems.get(0).getLastBooking()),
                () -> assertNotNull(allUserItems.get(0).getNextBooking())
        );
        verify(itemRepository, times(1)).findByOwnerIdOrderByIdAsc(userId);
    }
    ...
    void search_shouldReturnListItemDto() {
        ...
        assertAll(
                ...
                () -> assertEquals(itemMapper.toItemDto(item1), itemsFound.get(0))
        );
    ...
    void addComment_shouldReturnCommentOutputDto() {
        ...
        Mockito.when(bookingRepository.findByBookerIdAndEndIsBefore(anyInt(), any(LocalDateTime.class), any(Pageable.class)))
                .thenReturn(new PageImpl<>(List.of(lastBooking), Pageable.unpaged(), 1)); // Используем Pageable.unpaged()
    ...
    void addComment_shouldThrowUnavailableItemBookingException() {
        ...
        Mockito.when(itemRepository.findById(itemId)).thenReturn(Optional.of(item1));
        Mockito.when(bookingRepository.findByBookerIdAndEndIsBefore(anyInt(), any(LocalDateTime.class), any(Pageable.class)))
                .thenReturn(new PageImpl<>(List.of(), Pageable.unpaged(), 0)); // Эмулируем отсутствие бронирований
...
//        Integer userId = user1.getId();
//
//        Mockito.when(itemRepository.findByOwnerIdOrderByIdAsc(userId))
...
//                () -> assertNotNull(allUserItems.get(0).getNextBooking())
//        );
//        verify(itemRepository).findByOwnerIdOrderByIdAsc(userId);
//    }
//
//    @Test
//    void search_shouldReturnListItemDto() {
